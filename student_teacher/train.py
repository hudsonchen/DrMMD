...
def make_flags(args,config_file):
    if config_file:
        ...
        dic.update(config)
    if args.with_noise == 'true':
        args.with_noise = True
    else:
        args.with_noise = False
    # args.lr = args.lr * min(args.lmbda, 1.0)
    return args
...
parser.add_argument('--total_epochs', default=15000, type=int, help='total number of epochs')
...
parser.add_argument('--lmbda', default=1.0, type=float, help='lambda')
parser.add_argument('--with_noise', type= str, help='to use noise injection set to true')
